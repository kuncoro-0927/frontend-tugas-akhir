...
const ModalDeleteUsers = ({ open, handleClose, userId, onUpdate }) => {
  const handleDelete = async (e) => {
    e.preventDefault();

    try {
      const res = await instanceAdmin.delete(`/delete/users/${userId}`);
      ...
      // Log detail error
      console.error("Gagal menghapus pengguna. Detail error:");
        ...
        <div>
          ...
          >
            ...
            <div className="px-5 pb-2">
              <h2 className="text-2xl font-bold ">Hapus Pengguna</h2>
            </div>
          </div>
          ...
          <div className="px-6 mt-5 flex flex-col justify-center items-center pb-6">
            ...
            <p className="text-center mx-14 text-sm text-graytext">
              Pengguna yang dihapus tidak dapat dipulihkan. Pastikan Anda telah
              memeriksa kembali sebelum melanjutkan.
            </p>
          </div>

          <div className="flex justify-between mt-5 px-6 pb-4">
            <button onClick={handleClose} className=" rounded-md text-black ">
              Batal
            </button>
            <button
